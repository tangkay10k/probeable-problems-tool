...
/**
 * This class is to be used to run an input class against lecturer predefined unit tests. It will also be used to
 * validate a student's submission against the model answer (expected.txt).
 * <p>
 * Since we are restricted to java standard SDK, test suites need to be written in a particular format:
 * <p>
 * int testFoo() {
 ...
 * }
 * <p>
 * This marker class will then invoke all test cases in the test suite class, and redirect stdout and capture the
 * return values of each test case to be written to a txt file. If it is a student's submission, it will be validated
...
public class Marker {
    ...
    private static void captureStdoutAndReturnValues(int testNumber, Object testSuite, Method testcase)
        ...
        // TO DO: Allow new file to be created either as expected (student attempt) or output-problem-#no. (lecturer)
